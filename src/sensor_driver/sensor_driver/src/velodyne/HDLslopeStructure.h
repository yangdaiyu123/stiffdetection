...
class LidarProcess
{
    public:
		...
		boost::function<void (const pcl::PointCloud<pcl::PointXYZI>::ConstPtr&)>* fpointcloud_cb=NULL)
            : grabber_H_(grabber_H)
              ...
              , refineogm(40,40,0.2)
              , minzogm(40,40,1)
              , mindxdfineogm(70,40,0.2,20,20)
              , minzrefineogm(70,40,0.2,20,20)
              , minztrefineogm(70,40,0.2,20,20)
              , minyrefineogm(70,40,0.2,20,20)
              , mindtafineogm(70,40,0.2,20,20)
              , mindydfineogm(70,40,0.2,20,20)
              , slope5xogm(70,40,0.2,20,20)
              , slope5yogm(minzrefineogm.ogmheight,minzrefineogm.ogmwidth,minzrefineogm.ogmresolution)
              , slopemyogm(70,40,0.2,20,20)
	          ...
	          , fpointcloud_cb_(fpointcloud_cb)
    {
        ...
        const OGMData<float>* slopeprocess (std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> pointcloud);
	  ...
	  static void OGM2Mat(const OGMData<T>& ogmdata,cv::Mat& img)
	{
	  ...
	  cv::flip(img,img,0);
        ...
        template<class T>
                    void showdtaOGM(const char* windowname , const OGMData<T>& ogmdata);//mdj
        void haarGradient(const OGMData<float>& minzogm,OGMData<float>& slopexogm, OGMData<float>& slopeyogm , int window_halfnum, float slopeminthreshold , float slopemaxthreshold);
        void circletangentialDetection(const OGMData<float>& mindtaogm, const OGMData<float>& minzogm , const OGMData<float>& mindydogm , const OGMData<float>& mindxdogm , OGMData<float>& slopeyogm ,int window_halfxnum, int window_halfynum ,
                		int window_halfnum , float dtaaminthreshold = 26 );
        void haarProcess();
        void circletangentialprocess();
        void initHDL();
    ...
    private:
        ...
        OGMData<float> minztrefineogm;
        OGMData<float> minyrefineogm;
        OGMData<float> mindtafineogm;
        OGMData<float> mindydfineogm;
        OGMData<float> mindxdfineogm;
